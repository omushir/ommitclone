...
const Sprite = ({ id, x, y, direction, scripts, isPlaying, updateSprite, color, spriteType }) => {
  const spriteRef = useRef(null);
  const movementRef = useRef({ remaining: 0, dx: 0, dy: 0 });
      ...
      scripts.forEach(script => {
        switch (script.type) {
          case 'motion-move':
            ...
            const newX = x + Math.cos(radians) * steps;
            const newY = y - Math.sin(radians) * steps;
  ...
  }, [isPlaying, scripts, direction, x, y, id, updateSprite]);
  ...
  const renderSpriteContent = () => {
    if (spriteType === 'bird') {
      return <BirdSprite styles={{ width: '100%', height: '100%' }} />;
    } else {
      return (
        <svg
